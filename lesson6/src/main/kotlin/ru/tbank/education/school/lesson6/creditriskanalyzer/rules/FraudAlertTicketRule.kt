...
class FraudAlertTicketRule(
    ...
    override fun evaluate(client: Client): ScoringResult {
        ...
        var isFraudAlerts = false
        var isUnresolvedFraudAlerts = false
        for (ticket in tickets) {
            if (ticket.topic == TicketTopic.FRAUD_ALERT) {
                isFraudAlerts = true
                if (!ticket.resolved) { isUnresolvedFraudAlerts = true; break }
            }
        }
        val risk = when {
            isUnresolvedFraudAlerts -> PaymentRisk.HIGH
            isFraudAlerts -> PaymentRisk.MEDIUM
