...
class LoanDebtRatioRule(
    ...
    override fun evaluate(client: Client): ScoringResult {
        val totalDebt: Long = loanRepo.getLoans(client.id).filter { !it.isClosed }.sumOf { it.debt }
        val totalBalance: Long = accountRepo.getAccounts(client.id).sumOf { it.balance }

        val risk = when {
            totalBalance == 0L && totalDebt > 0L -> PaymentRisk.HIGH
