...
class HighRiskCategorySpendingRule(
    ...
    override fun evaluate(client: Client): ScoringResult {
        ...
        val sumGambling: Long = transactions.filter { it.category == TransactionCategory.GAMBLING }.sumOf { it.amount }
        val sumCrypto: Long = transactions.filter { it.category == TransactionCategory.CRYPTO }.sumOf { it.amount }
        val sumTransfer: Long = transactions.filter { it.category == TransactionCategory.TRANSFER }.sumOf { it.amount }
        val sumWithoutSalary: Long = transactions.filter { it.category != TransactionCategory.SALARY }.sumOf { it.amount }
        var ratio = 0.0
