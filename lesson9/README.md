...
```bash
grep -r "ERROR" logs/ logs/old > logs/error.txt
```
...
```bash
# Я не знаю почему, но в папке logs, у меня вообще нет .log, только .logs, а по заданию просят .log, но я поменял строчку "*.log" на "*.logs" иначе у меня не работает
find -name "*.logs" | wc -l > logs_const.txt
cat logs_const.txt
```
...
```bash
pacman -S zip # Если не установлена, у меня лично пришлось скачать
...
```bash
zip -r project_backup.zip config/*.conf logs -i "*.logs" errors.txt
```
...
```bash
# Я точно не понял учитывается ли пустая строка с пробелом, но я как понял нужно именно просто пустые строки без символов
