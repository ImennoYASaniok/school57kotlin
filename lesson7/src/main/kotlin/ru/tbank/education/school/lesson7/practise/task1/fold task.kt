...
fun buildSalaryReport(employees: List<Employee>): SalaryReport {
    if (employees.size == 0) return SalaryReport(0.0, 0.0, 0)
    val totalSalary: Double = employees.fold(0.0) { sum, el -> sum + el.salary }
    val avgSalary: Double = totalSalary / employees.size
    val departmentsCount = employees.map { it.department }.distinct().size
